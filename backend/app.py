...
def huffman_decompress(compressed_bytes, codebook, padding):
    ...
    # Convert bytes to bitstream and remove padding
    ...
    if not decoded_bytes:
        raise ValueError("No valid codes found in bitstream")
...
def upload_file():
    ...
    try:
        ...
        output_filename = f"{filename}_compressed{os.path.splitext(filename)[1]}"
        ...
        # Save codebook and padding info as JSON
        codebook_filename = f"{filename}_codebook.json"
        ...
        return jsonify({
            ...
            "codebook_filename": codebook_filename,  # Include codebook filename
